@extends('layouts.chapter_one_layout')
...
<div id = "game_prompt_assessment_result_id" class = "game_prompt_assessment_result_container customize_hide_element">
    ...
    <div class = "game_prompt_assessment_result_content">
        ...
        <button id = "game_assessment_result_revert_id" data-assessment-revert-current-scene-id = "" class = "game_prompt_assessment_result_content_button customize_prompt_assessment_result_backgroundgreen customize_hide_element" type = "button">Go back to Chapter 1 <i class="fa fa-undo"></i></button>
...
$(document).ready(function()
{
    ...
    $('.object_dialog_image').click(function()
    {
        ...
        else
        {
            ...
            for(i = 0; i < global_chapter_list['chapter_one']['chapter_scenes'].length; i++)
            {
                if(chapter_scene_id == global_chapter_list['chapter_one']['chapter_scenes'][i]['id'])
                {
                    for(j = 0; j < global_chapter_list['chapter_one']['chapter_scenes'][i]['find_object'].length; j++)
                    {
                        if(id == global_chapter_list['chapter_one']['chapter_scenes'][i]['find_object'][j]['id'])
                        {
                            $('div[data-object-dialog-id="'+ id +'"]').text(global_chapter_list['chapter_one']['chapter_scenes'][i]['find_object'][j]['message']);

                            global_chapter_list['chapter_one']['chapter_scenes'][i]['find_object'][j]['is_read'] = 1;
                        }
                    }
                }
            }
    ...
    $('#game_scene_previous_id').on('click', function()
    {
        ...
        for(i = 0; i < global_chapter_list['chapter_one']['chapter_scenes'].length; i++)
        {
            global_chapter_list['chapter_one']['chapter_scenes'][i]['is_current_selected'] = 0;
        }

        for(i = 0; i < global_chapter_list['chapter_one']['chapter_scenes'].length; i++)
        {
            let id = global_chapter_list['chapter_one']['chapter_scenes'][i]['id'];

            if(current_scene == id)
            {
                global_chapter_list['chapter_one']['chapter_scenes'][i]['is_read'] = 1;
            }

            if(next_scene == id)
            {
                global_chapter_list['chapter_one']['chapter_scenes'][i]['is_current_selected'] = 1;
            }
        }
    ...
    $('#game_scene_next_id').on('click', function()
    {
        ...
        for(i = 0; i < global_chapter_list['chapter_one']['chapter_scenes'].length; i++)
        {
            ...
            if(current_scene == id)
            {
                ...
                if(next_scene_type == "is_poem_question")
                {
                    global_chapter_list['chapter_one']['chapter_scenes'][i]['poem_question_and_answer']['answer'] = $('#game_question_text_id').val();
                }
            }
    ...
    $('#game_assessment_next_id').on('click', function()
    {
        ...
        if(is_chosen == true)
        {
            for(i = 0; i < global_chapter_list['chapter_one']['chapter_scenes'].length; i++)
            {
                for(j = 0; j < global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'].length; j++)
                {
                    global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'][j]['is_current_selected'] = 0;
                }
            }
            ...
            for(i = 0; i < global_chapter_list['chapter_one']['chapter_scenes'].length; i++)
            {
                for(j = 0; j < global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'].length; j++)
                {
                    let id = global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'][j]['id'];

                    if(current_scene == id)
                    {
                        global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'][j]['is_read'] = 1;

                        if(letter_chosen == "a")
                        {
                            global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'][j]['answer'] = letter_chosen;
                        }
                    ...
                    if(next_scene == id)
                    {
                        global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'][j]['is_current_selected'] = 1;
                    }
                }
            }
    ...
    $('#game_assessment_result_revert_id').on('click', function()
    {
        ...
        for(i = 0; i < global_chapter_list['chapter_one']['chapter_scenes'].length; i++)
        {
            global_chapter_list['chapter_one']['chapter_scenes'][i]['is_read'] = 0;
            global_chapter_list['chapter_one']['chapter_scenes'][i]['is_current_selected'] = 0;
        }

        for(i = 0; i < global_chapter_list['chapter_one']['chapter_scenes'].length; i++)
        {
            if(global_chapter_list['chapter_one']['chapter_scenes'][i]['poem_question_and_answer'] != null)
            {
                global_chapter_list['chapter_one']['chapter_scenes'][i]['poem_question_and_answer']['answer'] = '';
            }

            for(j = 0; j < global_chapter_list['chapter_one']['chapter_scenes'][i]['find_object'].length; j++)
            {
                global_chapter_list['chapter_one']['chapter_scenes'][i]['find_object'][j]['is_read'] = 0;
                global_chapter_list['chapter_one']['chapter_scenes'][i]['find_object'][j]['is_current_selected'] = 0;
            }

            for(j = 0; j < global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'].length; j++)
            {
                global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'][j]['answer'] = '';
                global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'][j]['is_read'] = 0;
                global_chapter_list['chapter_one']['chapter_scenes'][i]['assessment'][j]['is_current_selected'] = 0;
            }
        }
    ...
    {
        global_chapter_list['chapter_two']['is_chapter_unlocked'] = 1;
    ...
    function get_user_chapter()
    {
        $.ajax({
            ...
            success: function(result)
            {
                ...
                {
                    let chapter_list = result['data']['chapter_one']['chapter_scenes'],
                        chapter_one_answer_key_list = result['data']['chapter_one']['assessment_answer_keys'];
                    ...
                    if(chapter_list.length > 0)
                    {
                        for(i = 0; i < chapter_list.length; i++)
                        {
                            ...
                            if(is_read == 0)
                            {
                                chapter_process(chapter_scene_id, chapter_list, chapter_one_answer_key_list);
                            }
                            else
                            {
    ...
    function chapter_process(chapter_scene_id, chapter_list, chapter_one_answer_key_list)
    {
        for(i = 0; i < chapter_list.length; i++)
        {
            ...
            if(id == chapter_scene_id)
            {
                ...
                else if(is_assessment_question == 1)
                {
                    ...
                    if(assessment_all_answered == true)
                    {
                        ...
                        for(j = 0; j < assessment.length; j++)
                        {
                            for(k = 0; k < chapter_one_answer_key_list.length; k++)
                            {
                                if(assessment[j]['id'] == chapter_one_answer_key_list[k]['id'])
                                {
                                    if(assessment[j]['answer'] == chapter_one_answer_key_list[k]['answer'])
                                    {
                        ...
                        for(j = 0; j < chapter_one_answer_key_list.length; j++)
                        {
                            answer_keys += chapter_one_answer_key_list[j]['id'] + '. ' + chapter_one_answer_key_list[j]['answer'].toUpperCase() + ' ';
                        }

                        if(total_correct_answer >= 6)
                        {
                            $('#game_assessment_result_title_id').html('EXCELLENT! Proceed to chapter 2');
                        ...
                        {
                            $('#game_assessment_result_title_id').text('Oh! Seems like you were in a hurry. Let’s give it another shot.');
